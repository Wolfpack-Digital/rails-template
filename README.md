...
```
-d postgresql
-m https://raw.githubusercontent.com/Wolfpack-Digital/rails-starter-kit/master/template.rb
```
...
Normally, Rails only allows a single file to be specified as an application template (i.e. using the `-m <URL>` option). To work around this limitation, the first step this template performs is a `git clone` of the `Wolfpack-Digital/rails-starter-kit` repository to a local temporary directory.
